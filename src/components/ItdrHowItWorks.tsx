...
const DATA: Record<string, Data> = {
  ...
  de: {
    ...
    ladderH: 'Die abgestufte Vergeltungsleiter',
    ladder: 'Vergeltung ist kumulativ und ROE-kontrolliert. L1–L2 bleiben auf Ihrem eigenen Server — Honeypots und Desinformation, die dem Angreifer falsche Daten liefern. L3–L4 handeln gegen den Angreifer nur über rechtmäßige Kanäle: Meldung an globale Threat-Intel-Netzwerke und Koordination der Abschaltung mit ISPs, CERTs und Strafverfolgungsbehörden. Der Offizier führt die vollständige Untersuchung durch — Forensik → Attribution → Infrastruktur — bevor irgendetwas über L1 hinausgeht, und trifft nie Dritte.',
    closing: 'Zwei Invarianten durchziehen das gesamte Kontur: Es ist air-gapped, und aktive Vergeltung erfolgt nur nach Doctrine und mit Ihrer Zustimmung — jede Aktion unveränderlich protokolliert.',
  },
  es: {
    h: 'Cómo funciona el contorno — de la señal al golpe',
  ...
  it: {
    ...
    steps: [
      ...
      { label: 'Gate ROE e rappresaglia graduata', desc: 'Prima di qualsiasi risposta attiva, l\'ufficiale verifica cinque gate di regole d\'ingaggio — gravità, confidenza di attribuzione, classe dell\'attaccante, se l\'attacco è in corso, e la vostra autorizzazione. Tutti superati → la scala graduata L1–L4. Un gate fallisce → limitato alla difesa passiva ed escalation a un umano. Questo è ciò che rende sicuro il colpo.' },
    ...
    ladderH: 'La scala di rappresaglia graduata',
    ladder: 'La rappresaglia è cumulativa e controllata da ROE. L1–L2 restano sul vostro server — honeypot e disinformazione che alimentano l\'attaccante con dati falsi. L3–L4 agiscono contro l\'attaccante solo tramite canali legali: segnalazione a reti globali di threat-intel e coordinamento del takedown con ISP, CERT e forze dell\'ordine. L\'ufficiale conduce l\'indagine completa — forense → attribuzione → infrastruttura — prima di qualsiasi cosa oltre L1, e non colpisce mai terze parti.',
    closing: 'Due invarianti attraversano l\'intero contorno: è air-gapped, e la rappresaglia attiva avviene solo secondo dottrina e con la vostra approvazione — ogni azione registrata in modo immutabile.',
  },
  ja: {
    ...
    steps: [
      ...
      { label: 'ROEゲートと段階的報復', desc: '能動的な対応の前に、オフィサーは5つの交戦規則ゲートを確認します——深刻度、帰属の信頼度、攻撃者クラス、攻撃が進行中か、そしてお客様の承認。すべて合格→段階的なはしごL1～L4。いずれかのゲートが失敗→受動防御にとどめ、人間にエスカレーション。これが反撃を安全にする仕組みです。' },
    ...
    ladderH: '段階的報復のはしご',
    ladder: '報復は累積的でROEにより制御されます。L1～L2はお客様自身のサーバー内にとどまります——攻撃者に偽データを与えるハニーポットと偽情報。L3～L4は合法的なチャネルを通じてのみ攻撃者に対して行動します：グローバルな脅威インテリジェンスネットワークへの報告、ISP・CERT・法執行機関との連携によるテイクダウン。オフィサーはL1を超える前に完全な調査——フォレンジック→帰属特定→インフラ——を実施し、第三者を攻撃することは決してありません。',
    closing: 'コンター全体を貫く2つの不変原則：エアギャップされていること、そして能動的な報復は常にドクトリンとお客様の承認のもとでのみ行われること——すべてのアクションが不変的に記録されます。',
  },
  uk: {
    h: 'Як працює контур — від сигналу до удару',
    topoH: 'Де це працює',
    ...
    steps: [
      ...
      { label: 'ROE-гейт і градуйований удар', desc: 'Перед будь-якою активною відповіддю офіцер перевіряє п\'ять гейтів правил застосування — severity, впевненість атрибуції, клас атакуючого, чи триває атака, і ваш дозвіл. Усі пройдено → градуйована драбина L1–L4. Будь-який не пройдено → стоп на пасивній обороні й ескалація людині. Саме це робить удар безпечним.' },
    ...
    ladderH: 'Градуйована драбина відплати',
    ladder: 'Удар відплати кумулятивний і обмежений ROE. L1–L2 залишаються на вашому сервері — пастки й дезінформація, що годують атакуючого хибними даними. L3–L4 діють проти атакуючого лише законними каналами: репорт у глобальні threat-intel мережі та координація takedown з провайдерами, CERT і правоохоронцями. Офіцер проводить повне розслідування — форензика → атрибуція → інфраструктура — перш ніж щось вище L1, і ніколи не б\'є по третіх сторонах.',
    closing: 'Через весь контур проходять два інваріанти: він ізольований (air-gapped), і активний удар відплати трапляється лише за доктриною та з вашого схвалення — кожна дія в незмінному лозі.',
  },
  sr: {
    h: 'Како функционише контур — од сигнала до удара',
    topoH: 'Где то ради',
    ...
    steps: [
      ...
      { label: 'ROE капија и градуирана одмазда', desc: 'Пре било које активне реакције, официр проверава пет капија правила ангажовања — озбиљност, поузданост приписивања, класу нападача, да ли је напад у току, и ваше овлашћење. Све прође → градуирана лестница L1–L4. Било која капија не прође → ограничење на пасивну одбрану и ескалација човеку. Управо то чини удар безбедним.' },
    ...
    ladder: 'Одмазда је кумулативна и ограничена ROE-ом. L1–L2 остају на вашем сопственом серверу — лажне мете и дезинформације које нападачу дају лажне податке. L3–L4 делују против нападача само законитим каналима: пријава глобалним threat-intel мрежама и координација уклањања са ISP-овима, CERT-овима и органима реда. Официр спроводи потпуну истрагу — форензика → приписивање → инфраструктура — пре било чега изнад L1, и никада не удара треће стране.',
  ...
  hi: {
    ...
    ladderH: 'स्तरीय प्रतिशोध सीढ़ी',
    ladder: 'प्रतिशोध संचयी और ROE-नियंत्रित है। L1–L2 आपके अपने सर्वर पर रहते हैं — हनीपॉट्स और गलत सूचना जो हमलावर को झूठा डेटा देती है। L3–L4 केवल वैध चैनलों के माध्यम से हमलावर के खिलाफ कार्य करते हैं: वैश्विक थ्रेट-इंटेल नेटवर्क को रिपोर्टिंग और ISPs, CERTs और कानून प्रवर्तन के साथ टेकडाउन का समन्वय। ऑफिसर L1 से आगे कुछ भी करने से पहले पूरी जांच चलाता है — फोरेंसिक्स → एट्रिब्यूशन → इन्फ्रास्ट्रक्चर — और कभी तीसरे पक्ष पर प्रहार नहीं करता।',
    closing: 'पूरे कॉन्टूर में दो अपरिवर्तनीय तत्व चलते हैं: यह एयर-गैप्ड है, और सक्रिय प्रतिशोध केवल सिद्धांत के तहत और आपकी स्वीकृति से होता है — हर कार्रवाई अपरिवर्तनीय रूप से लॉग की जाती है।',
  },
  tr: {
    ...
    steps: [
      ...
      { label: 'ROE kapısı ve kademeli misilleme', desc: "Herhangi bir aktif yanıttan önce, görevli beş angajman kuralı kapısını kontrol eder — şiddet, atıf güveni, saldırgan sınıfı, saldırının devam edip etmediği ve yetkilendirmeniz. Hepsi geçerse → kademeli merdiven L1–L4. Herhangi bir kapı başarısız olursa → pasif savunmaya sınırlandırma ve bir insana eskalasyon. Vuruşu güvenli kılan budur." },
    ...
    ladderH: 'Kademeli misilleme merdiveni',
    ...
    closing: 'Tüm kontur boyunca iki değişmez geçerlidir: air-gapped\'dir ve aktif misilleme yalnızca doktrin altında ve onayınızla gerçekleşir — her eylem değiştirilemez şekilde kaydedilir.',
  },
  ar: {
    ...
    steps: [
      ...
      { label: 'بوابة ROE والانتقام المتدرج', desc: 'قبل أي استجابة نشطة، يتحقق الضابط من خمس بوابات لقواعد الاشتباك — الخطورة، ثقة الإسناد، فئة المهاجم، ما إذا كان الهجوم مستمرًا، وتفويضك. إذا نجحت جميعها → السلم المتدرج L1–L4. إذا فشلت أي بوابة → الاقتصار على الدفاع السلبي والتصعيد إلى إنسان. هذا ما يجعل الضربة آمنة.' },
    ...
    ladderH: 'سلم الانتقام المتدرج',
    ladder: 'الانتقام تراكمي ومقيد بـ ROE. تبقى L1–L2 على خادمك الخاص — مصائد العسل والتضليل التي تغذي المهاجم ببيانات كاذبة. تتصرف L3–L4 ضد المهاجم فقط عبر قنوات قانونية: الإبلاغ لشبكات معلومات التهديد العالمية والتنسيق لإزالة البنية التحتية مع مزودي خدمة الإنترنت وCERT وجهات إنفاذ القانون. يجري الضابط التحقيق الكامل — الطب الشرعي → الإسناد → البنية التحتية — قبل أي شيء يتجاوز L1، ولا يضرب أطرافًا ثالثة أبدًا.',
    closing: 'يمر ثابتان عبر المحيط بأكمله: أنه معزول (air-gapped)، وأن الانتقام النشط لا يحدث إلا وفقًا للعقيدة وبموافقتك — يتم تسجيل كل إجراء بشكل غير قابل للتغيير.',
  },
  el: {
    ...
    steps: [
      ...
      { label: 'Πύλη ROE & διαβαθμισμένο αντίποινο', desc: 'Πριν από οποιαδήποτε ενεργή απόκριση, ο αξιωματικός ελέγχει πέντε πύλες κανόνων εμπλοκής — σοβαρότητα, εμπιστοσύνη απόδοσης, κατηγορία επιτιθέμενου, αν η επίθεση είναι σε εξέλιξη, και την εξουσιοδότησή σας. Όλες περνούν → η διαβαθμισμένη κλίμακα L1–L4. Οποιαδήποτε πύλη αποτυγχάνει → περιορισμός σε παθητική άμυνα και κλιμάκωση σε άνθρωπο. Αυτό κάνει το χτύπημα ασφαλές.' },
    ...
    closing: 'Δύο αμετάβλητα στοιχεία διαπερνούν όλο το περίγραμμα: είναι απομονωμένο (air-gapped), και το ενεργό αντίποινο συμβαίνει μόνο υπό δόγμα και με την έγκρισή σας — κάθε ενέργεια καταγράφεται αμετάβλητα.',
  },
};
